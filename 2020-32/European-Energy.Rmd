...
ggplot(top_bot) +
  geom_rect(aes(xmin = -44, ymin = n*4 - 1.4,
  ...
  geom_text(aes(-42.5, n*4, label = country_name), family = "JetBrains Mono Bold", color = "white", hjust = 0, size = 8.5, check_overlap = TRUE) +
  geom_text(aes(percent, n*2, label = paste(percent, "% ", sep = "")), family = "JetBrains Mono Medium", color = "white", hjust = 1, size = 4, check_overlap = TRUE) +
  annotate("text", 85, 17, label = "European\nRenewable Energy Production", family = "BlexMono Nerd Font Mono Bold", color = "black", hjust = 1, size = 12, lineheight = 0.9) +
  annotate("text", 1, -11.5, label = "Source: Eurostat Energy\nGraphic: Alex Norman", family = "BlexMono Nerd Font Mono Light", color = "black", hjust = 0, size = 4, lineheight = 0.9) +
  scale_x_continuous(breaks = seq(0, 80, 20), labels = seq(0, 100, 25)) +
